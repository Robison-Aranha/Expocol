...
export const ImageTextAnaliser = () => {
  ...
  const handlerFile = (event) => {
    ...
    if (isImg(file)) {
      ...
      setGlobalModal([
        ...globalModal,
        { message: "O arquivo escolhido não é uma imagem!!" },
      ]);
    }
  };
    ...
    if (fileSwitch == 1) {
      return (
        <div className="ImageTextAnaliser-action">
          ...
          <button onClick={() => setFileSwitch(3)}>Camera</button>
          <input
            type="file"
            name="file"
    ...
    if (fileSwitch != 1) {
      if (!imagePreview && fileSwitch == 3) {
        return (
          <>
            <button
              className="button-black button-clear"
              onClick={() => setFileSwitch(1)}
            >
              ...voltar
            </button>
            <button
              onClick={() =>
                setImagePreview(takePicture("video-imageTextAnaliser"))
              }
            >
              Tirar Foto
            </button>
        ...
        return (
          <>
            ...
            <button onClick={analiseImageTextService}>Proseguir</button>
          </>
        );
      }
    }
  ...
  const returnState = () => {
    if (!state) {
      return (
        <>
          {returnAction()}
          {imagePreview && fileSwitch != 1 ? (
            <img id="imagePreview" src={URL.createObjectURL(imagePreview)} />
          ) : null}
          {fileSwitch != 1 ? (
  ...
  return (
    <div
      className={
        "ImageTextAnaliser-section" + (ImageTextAnaliser ? " modal" : "")
      }
    >
